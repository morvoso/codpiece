#!/usr/bin/env bash
# Run the box's own benchmark against codpiece, unmodified.
#
# db2.py hard-codes http://127.0.0.1:8020 and has no host flag, so codpiece takes that
# port for the duration — prod is stopped inside the window anyway. The point is that
# the script is not adapted to codpiece in any way: whatever it needs, codpiece provides,
...
echo "codpiece up on 8020"

cd $HOME/llm/bench && timeout 3000 python3 db2.py --label codpiece --depths ${DEPTHS:-0,32000} --reps ${REPS:-1} --max-tokens 400
rc=$?
echo "db2 rc=$rc"
docker rm -f $NAME >/dev/null 2>&1
