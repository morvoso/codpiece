#!/usr/bin/env bash
# Generation-parity harness: codpiece vs llama-completion (b10423), greedy,
...
set -u
TANDEM=${TANDEM:-/tmp/codpiece}
MODEL=${MODEL:-$HOME/llm/models/qwen35-small/Qwen3.5-0.8B-BF16.gguf}
ORACLE_BIN=${ORACLE_BIN_DIR:-$HOME/llm/codpiece/llama.cpp-b10423/build/bin}
N=${1:-64}
...
codpiece_gen() { # $1 = raw prompt
  local wrapped="<|im_start|>user
$1<|im_end|>
<|im_start|>assistant
"
  # CODPIECE_SUBCMD=run (stateless reference) or gen (session/engine path)
  nice -n 19 "$TANDEM" "${CODPIECE_SUBCMD:-run}" "$MODEL" -p "$wrapped" -n "$N" -t 8 2>/dev/null \
  | sed -e 's/<|im_end|>//'
}
