...
fn main() {
    ...
    if !pc_in.exists() {
        std::fs::write(
            ...
            "prefix=@CMAKE_INSTALL_PREFIX@\n\
             ...
             Name: ggml\n\
             Description: ggml (codpiece vendored build)\n\
    ...
    // CODPIECE_NATIVE_CPU=1 for machine-local perf experiments.
    if env::var("CODPIECE_NATIVE_CPU").ok().as_deref() != Some("1") {
    ...
    if cuda {
        cfg.arg("-DGGML_CUDA=ON")
            ...
            // mechanism codpiece's cached decode graph exists to exploit — is
            // compiled out
            .arg("-DGGML_CUDA_GRAPHS=ON");
    }
    run(&mut cfg, "cmake configure");

    let jobs = env::var("CODPIECE_BUILD_JOBS").unwrap_or_else(|_| {
    ...
    let mut builder = bindgen::Builder::default()
        ...
        .layout_tests(false);
    if cuda {
        builder = builder.clang_arg("-DCODPIECE_CUDA");
    }
