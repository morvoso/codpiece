...
impl From<codpiece_gguf::GgufError> for ModelError {
    fn from(e: codpiece_gguf::GgufError) -> Self {
        ModelError::Gguf(e)
    }
}
...
impl Weights {
    ...
    pub fn can_sample_in_graph(&self) -> bool {
        !self.is_tensor_parallel() || std::env::var("CODPIECE_SPLIT_OUTPUT").is_err()
    }
