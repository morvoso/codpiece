...
pub enum Axis {
    ...
    /// output features)
    Axis1,
    /// split along ne[2] (used by codpiece's 3-D GDN state, one [S,S] matrix
...
/// use codpiece's own names (`cache_k_l<N>`, `cache_v_l<N>`, `cache_conv_l<N>`,
...
pub fn classify(name: &str, hp: &Hparams) -> Split {
    ...
    if let Some(rest) = name.strip_prefix("cache_") {
        ...
        return match kind {
            ...
            // codpiece's shaped tensors. These follow codpiece's own layouts:
            //
    ...
    let Some((_il, suffix)) = layer_of(name) else {
        ...
        return match name {
            ...
            // tensor to save memory. CODPIECE_SPLIT_OUTPUT=1 restores the
            // memory-optimal choice.
            "output.weight" if std::env::var("CODPIECE_SPLIT_OUTPUT").is_err() => {
                Split::mirrored()
            }
...
pub fn granularity(name: &str, hp: &Hparams, blck: i64, n_devices: usize) -> i64 {
    ...
    if hp.is_recurrent(il) {
        ...
        match suffix {
            ...
            // cache_* suffixes arrive as the bare kind; codpiece's conv state
...
mod tests {
    ...
    fn split_axes_match_llama_cpp() {
        ...
        // column split is still reachable via CODPIECE_SPLIT_OUTPUT
    ...
    fn split_state_conserves_every_element() {
        ...
        let cases: &[(&str, i64, i64)] = &[
            ...
            ("cache_k_l3", 1024, 32),
            ("cache_v_l3", 1024, 32),
            // codpiece layouts: conv [d_conv-1, conv_dim] splits ne[1];
